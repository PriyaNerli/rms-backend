...
//new
const getFoodsByTypeController = async (req, res) => {
  try {
    const { type } = req.params; // "veg" or "non-veg"
    if (!type || !["veg", "non-veg"].includes(type)) {
      return res.status(400).send({
        success: false,
        message: "Invalid food type. Use 'veg' or 'non-veg'.",
      });
    }
    ...
    res.status(200).send({
      success: true,
      message: `Foods of type ${type} retrieved successfully.`,
      totalFoods: foods.length,
      foods,
    });
  } catch (error) {
